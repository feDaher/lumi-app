...
export default function Contacts() {
  ...
  async function loadContacts(pageToLoad = 1) {
    if (loading) return;
  ...
  const handleSearchDebounced = useDebouncedCallback(async (term: string) => {
    if (!term.trim()) {
      loadContacts(1);
      return;
    }

    const res = await ContactService.search(term);
    setContacts(res);
    setHasMore(false);
  }, 500);
  ...
  <SafeAreaView className="flex-1 bg-[#FAF0E6]">
    ...
    <View className="px-4 pt-4">
      ...
      <View className="w-full flex-row items-center justify-between -mt-1 mb-3">
      ...
      <View className="flex-row items-center bg-white mb-3 px-3 py-2 rounded-xl border border-gray-200">
          ...
          >
            {editingIndex === index ? (
              <View className="flex-1 mr-2">
                ...
                <TextInput
                  value={formatPhone(`${item.ddd}${item.phone}`)}
                  onChangeText={(text) => {
              ...
              <View>
                ...
                <Text className="text-gray-700 font-semibold">
                  {formatPhone(normalizeContactNumber(item))}
                </Text>
              </View>
            )}
            ...
            {editingIndex === index ? (
              ...
              <View className="flex-row items-center space-x-2 mt-4">
                <TouchableOpacity
                  className="bg-green-500 px-3 py-1 rounded-full"
                  onPress={() => sendWhatsApp(item.ddd, item.phone)}
                >
                ...
                <TouchableOpacity
                  className="bg-green-500 px-3 py-1 rounded-full"
                  onPress={() => makeCall(item.phone, item.ddd)}
                >
            ...
            {menuVisibleIndex === index && (
              <View
                className="absolute top-5 right-3 bg-white border border-[#FF1C8D] rounded-md shadow-md w-32 z-50 elevation-[9999]"
              >
                <TouchableOpacity
